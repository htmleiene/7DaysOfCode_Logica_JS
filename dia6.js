...
function mostrarCompras() {
    ...
    document.querySelector('.remItem').style.display = "block";
}

document.querySelector('.remItem').addEventListener('change', function (event){
    if (event.target.name === 'radQuestion' && event.target.value === 'Sim') {
        document.getElementById('itemRemove').style.display = 'block';
        document.getElementById('removeItemBtn').style.display = 'block';
    } else {
        document.getElementById('itemRemove').style.display = 'none';
        document.getElementById('removeItemBtn').style.display = 'none';
    }
});

document.getElementById('itemRemove').addEventListener('change', function () {
    const itemToRemove = document.getElementById('itemRemove').value.trim();
    if (itemToRemove) {
        const itemIndex = listaDeCompras.findIndex(compra => compra.item.toLowerCase() === itemToRemove.toLowerCase());
        if (itemIndex !== -1) {
            listaDeCompras.splice(itemIndex, 1);
            alert(`${itemToRemove} foi removido da sua lista de compras.`);
            mostrarCompras();
        } else {
            alert("Item não localizado na lista.");
        }
